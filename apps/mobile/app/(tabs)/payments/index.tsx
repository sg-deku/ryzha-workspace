...
function PaymentRow({ item }: { item: any }) {
  const auditStyle = AUDIT_COLORS[item.auditStatus] ?? "bg-slate-100 text-slate-500"
  const [bg, txt] = auditStyle.split(" ")
      ...
      <View className="flex-row items-start justify-between">
        <View className="flex-1 mr-2">
          <Text className="font-semibold text-slate-900" numberOfLines={1}>
            {item.customerEmail ?? item.description ?? "Payment"}
          </Text>
          <Text className="text-slate-400 text-xs mt-0.5">
            {item.currency.toUpperCase()} · {new Date(item.createdAt).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric" })}
          </Text>
        </View>
        <View className="items-end gap-1">
          ...
          <View className={`px-2 py-0.5 rounded-full ${bg}`}>
            <Text className={`text-[10px] font-bold ${txt}`}>{item.auditStatus}</Text>
          </View>
        </View>
      </View>
    </View>
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      <View className="px-5 pt-4 pb-3 flex-row items-center justify-between">
        <Text className="text-2xl font-bold text-slate-900">Payments</Text>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="flex-1 items-center justify-center mt-20">
              ...
              <Text className="text-slate-400 mt-3 text-base">No payments yet</Text>
            </View>
          }
        />
      )}
