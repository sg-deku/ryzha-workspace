    ...
    >
      <View className="flex-row items-center gap-3">
        <View className="w-10 h-10 rounded-full bg-blue-100 items-center justify-center">
          <Text className="text-blue-700 font-bold text-base">{item.name.charAt(0).toUpperCase()}</Text>
        </View>
        <View className="flex-1">
          <Text className="font-semibold text-slate-900">{item.name}</Text>
          <Text className="text-slate-400 text-xs mt-0.5">{item.email ?? "No email"}</Text>
        </View>
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="flex-1 items-center justify-center mt-20">
              ...
              <Text className="text-slate-400 mt-3 text-base">No customers yet</Text>
            </View>
          }
        />
      )}
